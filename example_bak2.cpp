...
/*
	...
	struct CriDRQN: nn::Cloneable<CriDRQN>{
		...
		CriDRQN(int inputsz=4,int outputsz=1){
			conv1=nn::Conv2d(nn::Conv2dOptions(inputsize,hiddensize,{conv1w,conv1h}));
			conv2=nn::Conv2d(nn::Conv2dOptions(hiddensize,hiddensize,{conv2w,conv2h}));
			bat1=nn::BatchNorm2d(hiddensize);
			lstm1=nn::LSTM(nn::LSTMOptions((WIDTH-conv1w+1-conv2w+1)*(HEIGHT-conv1h+1-conv2h+1)*128,hidden_size));
			bat2=nn::BatchNorm1d(1);
			linact1=nn::Linear(nn::LinearOptions(hidden_size,hidden_size));
			linact2=nn::Linear(nn::LinearOptions(hidden_size,1));
			register_module("conv1",conv1);
			register_module("conv2",conv2);
		...
		tuple<Tensor,tuple<Tensor,Tensor>> forward(Tensor s,tuple<Tensor,Tensor> hidden){
			auto x=nn::functional::elu(conv1(s));
			x=nn::functional::elu(conv2(s));
			x=bat1(x);
			x=x.flatten(1,-1);
			x=x.unsqueeze(1);
//			x.print();fflush(stdout);
			...
			x=x.squeeze();
//			x=bat2(x);
			Tensor cri=nn::functional::elu(linact1(x));
//			cout<<act<<endl;
			cri=linact2(cri);
		...
		void reset(){
			...
			bat1=nn::BatchNorm2d(hiddensize);
			lstm1=nn::LSTM(nn::LSTMOptions((WIDTH-conv1w+1-conv2w+1)*(HEIGHT-conv1h+1-conv2h+1)*128,hidden_size));
			bat2=nn::BatchNorm1d(hidden_size);
			linact1=nn::Linear(nn::LinearOptions(hidden_size,hidden_size));
			linact2=nn::Linear(nn::LinearOptions(hidden_size,1));
			register_module("conv1",conv1);
			register_module("conv2",conv2);
